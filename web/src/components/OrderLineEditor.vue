  ...
  <div class="order-lines">
    <div class="picker">
      ...
      <div v-loading="searching" class="product-grid">
        ...
        >
          <div class="pic-wrap">
            <el-image v-if="resolvePic(p.pic)" :src="resolvePic(p.pic)" fit="cover" class="pic">
              <template #error>
        ...
        <div v-if="!searching && products.length === 0" class="empty-hint">输入关键词搜索商品，再选择规格</div>
      </div>
    </div>
...
.line-pic-fallback { width: 44px; height: 44px; border-radius: 6px; flex-shrink: 0; }
.info { padding: 8px; }
.name {
...
.line-product { display: flex; gap: 10px; align-items: center; }
.line-pic { width: 44px; height: 44px; border-radius: 6px; flex-shrink: 0; }
