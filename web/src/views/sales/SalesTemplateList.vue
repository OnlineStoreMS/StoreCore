...
const defaultForm = () => ({
  ...
  footerExtra: '以上金额仅供参考确认，请核对明细后到店办理',
  showSkuPic: true,
  showStorePhone: true,
...
async function load() {
  loading.value = true
  try {
    const data = await listReceiptTemplates(undefined, 1, 100, 'sales')
...
<template>
  <div>
    ...
    <el-dialog
      ...
      :title="editingId ? '编辑销售单模板' : '新建销售单模板'"
      width="680px"
      destroy-on-close
    >
      <el-form label-width="120px">
        ...
        <el-form-item label="单据标题">
          <el-input v-model="form.headerTitle" placeholder="如：销售单据" />
        </el-form-item>
        ...
        <el-form-item label="正式页尾">
          <el-input v-model="form.footerThanks" type="textarea" :rows="2" placeholder="正式销售单页尾（签字栏等）" />
        </el-form-item>
