...
async function remove() {
  try {
    const tips = [`确认删除销售单「${order.value?.orderNo}」？删除后不可恢复。`]
    if (order.value?.serviceOrderId) {
      tips.push(`将同时删除关联服务工单 ${order.value.serviceOrderNo || '#' + order.value.serviceOrderId}，及其关联收银订单（如有）。`)
    }
