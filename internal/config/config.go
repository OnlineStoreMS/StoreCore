...
func Load(path string) (*Config, error) {
	...
	if cfg.Database.SQLitePath == "" {
		cfg.Database.SQLitePath = "./data/storecore.db"
	}
	if cfg.Database.PostgresDSN == "" {
		cfg.Database.PostgresDSN = "host=127.0.0.1 user=postgres password=postgres dbname=storecore port=5432 sslmode=disable TimeZone=Asia/Shanghai"
	}
	...
	if cfg.Storage.PublicBaseURL == "" {
		if cfg.Storage.Driver == "minio" {
			cfg.Storage.PublicBaseURL = "http://127.0.0.1:9100/storecore"
		} else {
			cfg.Storage.PublicBaseURL = "http://127.0.0.1:8094/uploads"
		}
	}
